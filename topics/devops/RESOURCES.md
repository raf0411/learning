# RESOURCES

## Primary local references

- Ubuntu homelab manual pages: `man <command>`
- Command summaries for the installed version: `<command> --help`
- Installed package documentation: `/usr/share/doc/`

External official documentation will be added when each topic is introduced so
this file remains a short, useful reference rather than a link dump.

## Command discovery and shell data flow

- [GNU Bash: `type` and `help` builtins](https://www.gnu.org/software/bash/manual/html_node/Bash-Builtins.html)
- [GNU Bash: pipelines](https://www.gnu.org/software/bash/manual/html_node/Pipelines.html)
- [GNU Bash: redirections](https://www.gnu.org/software/bash/manual/html_node/Redirections.html)
- [GNU `grep` manual](https://www.gnu.org/software/grep/manual/grep.html)
- [GNU Coreutils: `head`](https://www.gnu.org/software/coreutils/manual/html_node/head-invocation.html)

## Filesystem hierarchy, editing, and archives

- [Filesystem Hierarchy Standard 3.0](https://refspecs.linuxfoundation.org/FHS_3.0/fhs-3.0.html)
- [Ubuntu `file-hierarchy(7)` manual](https://manpages.ubuntu.com/manpages/noble/man7/file-hierarchy.7.html)
- [GNU Nano manual](https://www.nano-editor.org/dist/latest/nano.html)
- [GNU Nano shortcut reference](https://www.nano-editor.org/dist/latest/cheatsheet.html)
- [GNU Tar manual](https://www.gnu.org/software/tar/manual/tar.html)

## Users, groups, permissions, and privilege

- [Linux `credentials(7)`](https://man7.org/linux/man-pages/man7/credentials.7.html)
- [Linux `inode(7)`](https://man7.org/linux/man-pages/man7/inode.7.html)
- [Linux `path_resolution(7)`](https://man7.org/linux/man-pages/man7/path_resolution.7.html)
- [GNU Coreutils: mode structure](https://www.gnu.org/software/coreutils/manual/html_node/Mode-Structure.html)
- [GNU Coreutils: setting permissions](https://www.gnu.org/software/coreutils/manual/html_node/Setting-Permissions.html)
- [GNU Coreutils: numeric modes](https://www.gnu.org/software/coreutils/manual/html_node/Numeric-Modes.html)
- [Linux `unlink(2)`](https://man7.org/linux/man-pages/man2/unlink.2.html)
- [Official `sudoers(5)` manual](https://www.sudo.ws/docs/man/1.9.14/sudoers.man.pdf)
- [NIST: least privilege](https://csrc.nist.gov/glossary/term/least_privilege)

## Ubuntu account administration

- [Ubuntu Server: user management](https://ubuntu.com/server/docs/how-to/security/user-management/)
- [Ubuntu 24.04 `adduser(8)`](https://manpages.ubuntu.com/manpages/noble/man8/adduser.8.html)
- [Ubuntu 24.04 `addgroup(8)`](https://manpages.ubuntu.com/manpages/noble/man8/addgroup.8.html)
- [Ubuntu 24.04 `useradd(8)`](https://manpages.ubuntu.com/manpages/noble/man8/useradd.8.html)
- [Ubuntu 24.04 `usermod(8)`](https://manpages.ubuntu.com/manpages/noble/man8/usermod.8.html)
- [Ubuntu 24.04 `passwd(1)`](https://manpages.ubuntu.com/manpages/noble/man1/passwd.1.html)
- [Ubuntu 24.04 `sudo(8)`](https://manpages.ubuntu.com/manpages/noble/man8/sudo.8.html)
- [Ubuntu 24.04 `nologin(8)`](https://manpages.ubuntu.com/manpages/noble/man8/nologin.8.html)
- [Ubuntu 24.04 `getent(1)`](https://manpages.ubuntu.com/manpages/noble/man1/getent.1.html)
- [Ubuntu 24.04 `systemd.exec(5)`](https://manpages.ubuntu.com/manpages/noble/man5/systemd.exec.5.html)
- [Ubuntu 24.04 `systemd.service(5)`](https://manpages.ubuntu.com/manpages/noble/man5/systemd.service.5.html)
- [Debian Policy: system users and groups](https://www.debian.org/doc/debian-policy/ch-opersys.html#users-and-groups)
